...
| 时间线 | 变更 | 架构支持 |
|--------|------|---------|
| 近期 (P0) | 播放器倍速/PiP/章节 UI | 仅前端 `VideoPlayerBox.vue` 改造 |
| 近期 (P0) | 字幕编辑器前端 | 新建 `SubtitleEdit.vue` + 时间轴组件 |
| 中期 (P1) | 评论排序/过滤 UI | `VdCommentPanelMb.vue` 改造 |
| 中期 (P2) | ASR 自动转写 Worker | `TaskLog.task_type = subtitle_asr` 已预留 |
| 中期 (P2) | 专题活动页系统 | `SpecialPage` / `Campaign` 模型已就绪 |
| 长期 (P3) | 直播模块 | 需引入 SRS/ZLMediaKit 流媒体服务器 |
| 长期 | 微服务拆分 | handler 文件边界即服务边界；引入 API 网关 |

### 扩展路径

当前容量: ~50 并发管理员，单实例  
→ 下一层级（500 并发）: 水平扩展 Gin 实例 + Nginx 负载均衡 + Redis session 共享  
→ 再下一层级（微服务）: 按 handler 文件拆分为独立 Kratos 服务 + Consul/Nacos 服务发现

### 重新审视触发条件（汇总自 ADR）

| 触发条件 | 应重新评估的决策 |
|---------|----------------|
| 管理员并发 > 200 | ADR-004: RBAC 权限缓存到 Redis |
