...
Cakecake 完整实现了视频分享平台的全链路核心功能。主要解决以下问题：

1. **用户认证与安全**：注册登录（唯一性校验）、个人信息维护、账号注销（7 天冷静期）。密码 bcrypt 加密，JWT 双 Token（用户 Access 2h + Refresh 30d；管理员 Access 2h + Refresh 3d）。
2. **视频内容管理**：上传（≤500MB, ≤30min）→ 异步转码（FFmpeg→H.264 MP4→OSS）→ 状态机（draft→processing→published/failed）→ Redis 播放计数 10s 落库。
3. **实时弹幕交互**：WebSocket 长连接，≤200ms 实时推送，Canvas 多轨道渲染，敏感词过滤，弹幕点赞，5s 冷却。
4. **多级评论系统**：视频/文章/动态三套独立评论表，3 级嵌套回复，UP 主管理（精选/关闭/置顶/级联删除），点赞/反对 toggle，待审隔离机制。
5. **社交互动体系**：关注/取关、拉黑/解除（双向互阻）、关注分组、多收藏夹、稍后再看、投币互动、图文动态发布。
6. **私信与通知系统**：一对一私信（WebSocket 实时推送 + 未读计数）、AI 智能助手对话、点赞聚合通知、评论回复通知、免打扰机制。
7. **硬币经济体系**：每日任务获取硬币、视频/文章投币消费、收支流水账本（coin_ledgers）、经验值等级（Lv1~Lv6）。
8. **搜索与历史**：ES 全文搜索 + 搜索历史 + 观看/阅读/直播历史追踪 + 中途续播进度 + 每日任务系统。
9. **直播系统**：UUID 推流密钥 → SRS 回调（on_publish/on_done）→ flv.js HTTP-FLV 播放 → WebSocket 实时聊天 + 6 种礼物 + 弹幕飘屏 + 管理员警告/封禁。
10. **Feed 推荐系统**：协同过滤召回 → MMR 多样性重排序（λ=0.7）→ 游标分页 → 分区推荐 + 排行榜（按播放/投币/收藏/周/月/全部）。
11. **风控引擎**：白名单优先 → 黑名单拦截 → 多层级规则匹配（关键词/正则/频率限制）→ 4 种 Action 分流（拦截/隔离/告警/封禁）→ 到期自动解封。
12. **运营后台**：23 个模块，全部前后端对齐。覆盖数据概览、内容审核、用户管理、工单风控、版权管理、数据报表、客服后台、运维监控、配置发布、权限审计、播放器高级、字幕管理、Feed 推荐。
...
| 直播流媒体 | SRS + flv.js | 5.0+ | RTMP 推流接收、HTTP-FLV 低延迟播放 |
| 视频处理 | FFmpeg | 7.0+ | H.264 编码转码、封面截帧 |
| 搜索引擎 | Elasticsearch（可选） | 8.15 | 全文搜索、热搜管理 |
| AI 集成 | 可以任意 API | - | AI 智能助手对话 |
| 鉴权机制 | JWT 双 Token | - | 用户 Access（2h）+ Refresh（30d），管理员 Access（2h）+ Refresh（3d） |

---

## 数据库设计

系统基于 MySQL 8.0 InnoDB（utf8mb4），GORM AutoMigrate 首次启动自动建表，共计 **85 张数据表**，按 15 个业务模块组织。
...
| E-R 图 | 说明 |
|--------|------|
| ![完整 ER 图](docs/images/er-diagram-full.png) | **85 实体 · Crow's Foot 标注 · Figma 风格** — [交互版](docs/cakecake_er_figma-diagram.html) |
...
| [cakecake_er_bento.html](docs/cakecake_er_bento.html) | **Bento 风格** 数据库架构总览（85 实体 / 15 模块） |
...
| 直播系统 | 创建/设置直播间 → UUID 推流密钥 → SRS 回调推流（on_publish/on_done） → flv.js HTTP-FLV 播放 → WebSocket 实时聊天 + 6 种礼物动画 → LiveDanmaku 弹幕飘屏 → 观众在线追踪 → 管理员警告/封禁 → 直播历史记录（见下方直播系统章节） |
| 推荐系统 | 协同过滤召回 → MMR 多样性重排序（λ=0.7）→ 游标分页 → 分区推荐 + 排行榜（按播放/投币/收藏） |
| 风控引擎 | 白名单优先 → 黑名单拦截 → 多层级规则匹配（关键词/正则/频率限制）→ reject/quarantine/notify_admin/auto_ban → 到期自动解封 |
...
| # | 模块 | 状态 | 功能 |
|---|------|:--:|------|
| 1 | 数据概览 | ✅ | 9 张概览卡片 + ECharts 图表（用户/视频/文章/评论/播放/弹幕） |
| 2 | 首页轮播 | ✅ | 横幅 CRUD + 排序 + 起止时间 |
| 3 | 热搜运营 | ✅ | 关键词 pin/block/manual 干预 + 布局排序 |
| 4 | 用户管理 | ✅ | 用户列表、封禁、信息编辑 |
| 5 | 视频审核 | ✅ | 审核 publish/reject → 记录审核人 + 时间 |
| 6 | 专栏审核 | ✅ | 文章内容审核 |
| 7 | 动态管理 | ✅ | 三表统一视图（视频+专栏+图文）+ 类型过滤 |
| 8 | 评论管理 | ✅ | 跨 3 表联合查询 + 待审隔离筛选 |
| 9 | 系统设置 | ✅ | 全局配置管理，同步更新 .env + 内存 |
| 10 | 举报处理 | ✅ | 多类型举报受理与批量处理 |
| 11 | AI 角色 | ✅ | Agent Profile CRUD + 提示词 + 欢迎语 + LLM 提供商 |
| 12 | 工单管理 | ✅ | 用户提交→处理→对话记录→满意度评价→SLA 自动升级 |
| 13 | 风控管理 | ✅ | 规则引擎（关键词/正则/频率限制）+ 黑白名单 + auto_ban 真封号 + 到期解封 |
| 14 | 版权管理 | ✅ | 投诉→审核→下架/驳回 + 反通知机制 |
| 15 | 数据报表 | ✅ | ECharts 多维度统计 + CSV 导出 + 报表配置保存 |
| 16 | 客服后台 | ✅ | 模板管理 + 会话管理 + 快捷回复 |
| 17 | 运维监控 | ✅ | 任务队列/告警/链路追踪/健康检查/CDN 刷新/存储生命周期 |
| 18 | 配置发布 | ✅ | 功能开关（FNV-1a Hash 灰度）+ 版本发布（快照→部署→回滚） |
| 19 | 权限审计 | ✅ | RBAC 角色/权限点 + 审计日志 + 审批流 + 登录日志 |
| 20 | 播放器高级 | ✅ | 视频章节标记 + 多码率版本管理 |
| 21 | 字幕管理 | ✅ | 字幕 CRUD + VTT/SRT 格式 + 内容预览 |
| 22 | 评论增强 | ✅ | 图片评论 + 评论举报处理 + 排序配置 |
| 23 | Feed 推荐 | ✅ | 个性化推荐流(MMR) + 订阅源 + 排行榜 + 分区推荐 |
...
MySQL 需先建库（如 `minibili`）；**表由首次启动时 GORM AutoMigrate 自动创建**（85 张表，含全部索引与约束），无需手动执行 SQL。
...
```
用户请求推荐 → ES 检索候选集（协同过滤 + 内容相似度）
  → MMR 重排序（最大化相关性 - λ × 各分区最大相似度）
  → 去重（排除已观看） → 分页返回（limit≤50, next_cursor 游标）
```

技术细节：

- **候选召回**：基于用户分区偏好 + 标签相似度的协同过滤，ES 返回 Top-N 候选
- **MMR 重排序**：`Score = Relevance(video, user) - λ × max(Sim(video, already_selected))`。λ 控制多样性强度，默认 0.7
- **分区推荐**：`/zones/:zone/recommendation` 在指定分区内执行 MMR，保障垂直内容曝光
- **订阅源**：`/feed/subscription` 纯时间序，cursor 游标分页（`id < cursor`），无多样性排序
- **排行榜**：`/leaderboard?by=play|coin|fav&period=week|month|all`，全量排序 Top-N
- **匿名用户**：Redis 缓存热门推荐，TTL 5min，减少重复计算
...
用户发表内容 → resolveContentOwner(定位作者)
  ...
  → 逐条匹配：
      keyword → strings.Contains(大小写不敏感)
      regex   → /pattern/ 格式识别 → regexp.Compile("(?i)") 编译 + 缓存
      rate_limit → risk_rate_counters 表窗口计数 vs max_count/window_sec
  → 命中 → 写 RiskHitLog → 按 Action 分流：
      reject       → 返回 blocked=true（调用方删除/拦截）
      quarantine   → 评论设 approved=false（comment/article/dynamic 全覆盖）
      notify_admin → ChatHub WebSocket 实时推送 + NotificationRecord 入库
      auto_ban     → User.Status="banned" + BanExpiresAt → 到期自动解封
```

关键技术：

- **白名单优先**：白名单用户绕过所有规则，不受任何拦截
- **正则缓存**：每个规则 ID 编译一次 `regexp.Regexp`，cache map 复用
- **频率窗口**：`risk_rate_counters` 表按 `(rule_id, user_id, window_start)` 维护独立计数
- **黑白名单过期**：`expires_at` 字段 + 后台 goroutine 每 60s 清理过期条目
- **到期解封**：同 goroutine 每 60s 检查 `ban_expires_at <= now`，自动回 `status=active`
- **隔离审阅**：评论管理页「待审」筛选直接查 `approved=false`，运营可二次确认后通过/删除

### 直播推流与互动

```
主播 OBS --RTMP--> [SRS / node-media-server]
                       |
                       | on_publish / on_done HTTP Callback
                       v
               [Go Gin API]
               /     |      \
         live.go  admin_live.go  ws.go
       (房间CRUD)  (审核管理)   (WebSocket聊天+礼物)
```

关键技术细节：

- **推流**：主播通过 OBS 推送 RTMP 流到 SRS，Go 后端通过 `/live/callback/on_publish` 回调校验 `stream_key`（UUID v4），拒绝已封禁房间
- **播放**：前端 `LivePlayer.vue` 使用 flv.js 拉取 HTTP-FLV 流（`http://<host>:8000/live/{stream_key}.flv`），`<video>` 自动播放，根据 `room.status` 启停
- **聊天**：WebSocket 端点 `GET /api/v1/ws/live?room_id=N&token=...`，进/离场系统消息、聊天转发（≤200字符）、观众列表实时同步
- **弹幕**：`LiveDanmaku.vue` CSS 动画飘屏，最多 40 条同屏，5-9s 随机动画时长，7 色随机
- **礼物**：6 种礼物（玫瑰/小心心/火箭/星星/蛋糕/花束），全屏特效动画 + `live_rooms.viewer_count` 增量
- **管理**：管理员可警告（WebSocket 广播 `admin_warning` → 前端 5s 覆盖层）或封禁（`admin_ban` → 前端 60s 遮罩），支持警告模板 CRUD
- **状态机**：`idle → live → ended`，任意状态可被管理员 → `banned`
- **断线重连**：WebSocket 断开后客户端 3 秒自动重试

### 直播数据表

| 表名 | 字段数 | 说明 |
|------|--------|------|
| `live_rooms` | 12 | 直播房间主表（标题、封面、UUID 推流密钥、状态机、观众数、开播/结束时间） |
| `live_warn_templates` | 5 | 管理员警告模板（预置文案、排序权重） |
| `live_view_histories` | 6 | 直播观看历史（用户+房间联合唯一索引，upsert 模式） |

### 直播 WebSocket 事件协议

| 方向 | Type | 字段 | 说明 |
|------|------|------|------|
| S → C | `user_info` | `username, users[], user_count, broadcaster` | 连接时发送自身信息+观众列表 |
| S → C | `audience` | `users[], count` | 观众列表增量更新 |
| S → C | `system` | `msg` | 系统消息（进场/离场/关注主播） |
| C → S | — | `{ "content": "文字" }` | 发送聊天消息 |
| C → S | — | `{ "gift": "rose\|heart\|rocket\|star\|cake\|flower" }` | 发送礼物 |
| C → S | — | `{ "follow": true }` | 关注主播 |
| S → C | `gift` | `username, gift, label` | 礼物广播（全房间可见） |
| S → C | `admin_warning` | `reason, msg` | 管理员警告覆盖层 |
| S → C | `admin_ban` | `msg` | 管理员封禁通知 |
...
| [docs/cakecake_er_figma-diagram.html](./docs/cakecake_er_figma-diagram.html) | 开发 | 完整 ER 图（85 实体，Crow's Foot 标注） — 交互版 |
| [docs/AdminER_Diagram.html](./docs/AdminER_Diagram.html) | 开发 | 扩展模块 ER 图（39 表） — 交互版 |
| [docs/cakecake_er_bento.html](./docs/cakecake_er_bento.html) | 开发 | Bento 风格 数据库架构总览（85 实体 / 15 模块） |
