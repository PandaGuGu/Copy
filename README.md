...
> 项目灵感来源于 [earthcake2233/cakecake](https://github.com/earthcake2233)，在此基础上完成了数据库重构、运营后台全面扩建（82 张数据表，23 个后台模块），形成了可投入生产级使用的完整系统。维护仓库：[PandaGuGu/Copy](https://github.com/PandaGuGu/Copy)。
...
| AI 集成 | DeepSeek API | V3 | AI 智能助手对话 |
...
系统基于 MySQL 8.0 InnoDB（utf8mb4），GORM AutoMigrate 首次启动自动建表，共计 **82 张数据表**，按 14 个业务模块组织。
...
| E-R 图 | 说明 |
|--------|------|
| ![完整 ER 图](docs/images/er-diagram-full.png) | **82 实体 · Crow's Foot 标注 · Figma 风格** — [交互版](docs/cakecake_er_figma-diagram.html) |
...
| [cakecake_er_bento.html](docs/cakecake_er_bento.html) | **Bento 风格** 数据库架构总览（82 实体 / 14 模块） |
...
| 历史记录 | 4 | `video_view_histories`, `article_view_histories`, `user_search_histories`, `user_daily_tasks` |
...
| 搜索模块 | ES 全文搜索 → 搜索历史 → 观看/阅读历史 → 中途续播进度追踪 → 每日任务 |
...
MySQL 需先建库（如 `minibili`）；**表由首次启动时 GORM AutoMigrate 自动创建**（82 张表，含全部索引与约束），无需手动执行 SQL。
...
| [docs/cakecake_er_figma-diagram.html](./docs/cakecake_er_figma-diagram.html) | 开发 | 完整 ER 图（82 实体，Crow's Foot 标注） — 交互版 |
| [docs/AdminER_Diagram.html](./docs/AdminER_Diagram.html) | 开发 | 扩展模块 ER 图（39 表） — 交互版 |
| [docs/cakecake_er_bento.html](./docs/cakecake_er_bento.html) | 开发 | Bento 风格 数据库架构总览（82 实体 / 14 模块） |
